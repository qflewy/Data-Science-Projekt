...
def show_median_price_heatmap_per_region(
    region_price_path: Path,
    region_path: Path,
    year: int,
    fuel_type: str,
    monthly_median_prices: Path = None,
    use_median_file: bool = False,
):
    """This function plots a map that dynamically projects the monthly median
    prices of a selected fuel type over a selected year for all post code
    Leitregionen.

    If the monthly medians file exists, set use_median_file to True and
    give the filepath as input for monthly_median_prices to avoid having
    to compute the values from scratch.
    Returns nothing.
    i: Path region_price_path, Path region_path, int year, string
    fuel_type, Path monthly_median_prices, bool use_median_file
    o: None
    """
    # for website: if the median price file exists,
    # use it to avoid computational overhead.
    if use_median_file & monthly_median_prices.exists():
        ...
        for region in regions:
            file_path = (
                region_price_path / f"mean_median_price_region_{region}.csv"
            )
            ...
            if file_path.exists():
                ...
                processed_region_lf = (
                    ...
                    .agg(
                        pl.col(f"{fuel_type}_median")
                        .median()
                        .alias(f"{fuel_type}_median_price")
                    )
                )
        ...
        # join prices with regions to get the coordinates
        # for the respective median price.
        display_df = global_df.join(
            df_regions, left_on="region", right_on="leit_plz", how="left"
        ).sort("month")
...
def get_borderregion_stations(station_input_path: Path, outputpath: Path):
    ...
    Creates a Dataframe with the border and surrounding stations and
    their closest neighbouring country.
    This function uses the open-street-maps osmx api for the border
    coordinates.
    ...
    station_df = pd.read_csv(
        station_input_path,
        usecols=["uuid", "latitude", "longitude", "brand", "post_code"],
    )

    # convert coordinates into geodata
    # to be able to calculate with distance in meter.
    print("converting into geodata")
    station_geo_df = gpd.GeoDataFrame(
        station_df,
    ...
    # projection into metric system.
    station_geo_df = station_geo_df.to_crs(epsg=32632)
    ...
    # loading the borders of neighbouring countries
    # and calculating the distance from the stations.
    dist_cloumns = []
    ...
    for country in neighbor_countries:
        ...
        station_geo_df[col_name] = (
            station_geo_df.geometry.distance(geom) / 1000
        )
        dist_cloumns.append(col_name)

    # get closest border for each statino
    # and extract country name and minimum distance.
    station_geo_df["closest_border_col"] = station_geo_df[dist_cloumns].idxmin(
        axis=1
    )
    station_geo_df["neighbour_country"] = station_geo_df[
        "closest_border_col"
    ].str.replace("dist_", "")
    station_geo_df["dist_km"] = station_geo_df[dist_cloumns].min(axis=1)

    # classify each station into either border,
    # border surrounding region or far away (inland)
    # (-> to avoid regional differences we ignore the latter).
    ...
    mask_dk = (station_geo_df["neighbour_country"] == "Denmark") & (
        (station_geo_df["longitude"] > 9.6)
        | (station_geo_df["longitude"] < 8.8)
    )
    mask_bodensee = (
        (station_geo_df["neighbour_country"].isin(["Switzerland", "Austria"]))
        & (station_geo_df["latitude"] < 47.8)
        & (station_geo_df["longitude"] > 8.9)
        & (station_geo_df["longitude"] < 9.8)
    )
    station_geo_df.loc[mask_dk | mask_bodensee, "border_region"] = (
        "sea border (ignore)"
    )

    # filter to only keep border and surrounding stations
    # & cleanup to save to csv.
    filter_stations_gdf = station_geo_df[
        station_geo_df["border_region"].isin(
            ["Border (0-8km)", "Surrounding (8-25km)"]
        )
    ].copy()
    necessary_columns = [
        "uuid",
    ...
    print(
        f"number of found border and surrounding stations:: {len(export_df)}"
    )
...
def mann_whitney_test_border_prices(
    ...
    """Performs a Mann-Whitney-U-Test that tests whether two independent
    samples (border stations and non-border stations) have the same
    distribution.

    Returns two pandas DataFrames, one for the overall results and one
    for the separate yearly results.
    ...
    border_lf = pl.scan_csv(border_stations_file).with_columns(
        pl.col("uuid").cast(pl.Utf8)
    )

    price_lf = pl.scan_parquet(price_file).with_columns(
        pl.col("station_uuid").cast(pl.Utf8)
    )

    preprocessed_lf = price_lf.with_columns(
        pl.col("day").dt.year().alias("year")
    ).join(border_lf, left_on="station_uuid", right_on="uuid", how="inner")

    # get df for yearly test.
    yearly_lf = preprocessed_lf.group_by(
        ["year", "neighbour_country", "border_region", "station_uuid"]
    ).agg(
        pl.col(f"{fuel_type}_median")
        .median()
        .alias(f"{fuel_type}_median_price")
    ...
    overall_lf = preprocessed_lf.group_by(
        ["neighbour_country", "border_region", "station_uuid"]
    ).agg(
        pl.col(f"{fuel_type}_median")
        .median()
        .alias(f"{fuel_type}_median_price")
    ...
    def __calculate_test(df: pl.DataFrame, country: str):
        ...
        border_prices = (
            df_country.filter(pl.col("border_region") == "Border (0-8km)")
        ...
        surrounding_prices = (
            df_country.filter(
                pl.col("border_region") == "Surrounding (8-25km)"
            )
            .get_column(f"{fuel_type}_median_price")
            .cast(pl.Float64, strict=False)
            .drop_nans()
        ...
        # (-> checks for differences on both sides).
        _, p_value = stats.mannwhitneyu(
            border_prices, surrounding_prices, alternative="two-sided"
        )
        ...
        return {
            ...
            # negative means that border stations are cheaper.
            "Price_difference": round(median_border - median_surrounding, 3),
            "p_value": p_value,
    ...
    yearly_result_df = yearly_result_df[
        [
            "year",
            "Country",
            "Median_border",
...
def get_autobahn_stations(
    station_input_path: Path, autobahn_output_path: Path
):
    """This functions filters manually for buzzwords that indicate whether a
    gas station is on the Autobahn.

    It saves the Autobahn stations dataframe as a csv to the given output path.
    Returns nothing.
    ...
    buzzword_pattern = (
        r"(?i)(?:bab|raststätte|autobahn|rastanlage|rasthof|\bA\s?\d{1,3}\b)"
    )

    # create masks for name, street & house number
    # (because for some entries, the autobahn was
    # in the house_number column).
    # this part was written with the help of gemini.
    mask_name = stations_df["name"].str.contains(
        buzzword_pattern, regex=True, na=False
    )
    mask_street = stations_df["street"].str.contains(
        buzzword_pattern, regex=True, na=False
    )
    mask_house_number = stations_df["house_number"].str.contains(
        buzzword_pattern, regex=True, na=False
    )
...
def filter_autobahn_from_borders(
    border_file: Path, autobahn_file: Path, output_path: Path
):
    """This method removes all Autobahn stations from the border region
    stations.

    The result is saved as a new dataframe as csv to the given outputpath.
    Returns nothing.
    i: Path border_file, Path autobahn_file, Path output_path
    o: None
    """
    autobahn_df = pl.read_csv(autobahn_file)
    border_df = pl.read_csv(
        border_file, schema_overrides={"post_code": pl.Utf8}
    )

    # with the anti-join, all stations that are in the autobahn
    # stations df are getting removed from the border stations df.
    no_autobahn_border_df = border_df.join(autobahn_df, on="uuid", how="anti")

    # (named "lower" because the threshold for border distance
    # was lowered in the course of the project.)
    no_autobahn_border_df.write_csv(
        output_path / "lower_non_autobahn_border_stations.csv"
    )


def show_border_price_difference(
    median_price_path: Path,
    border_stations_file: Path,
    fuel_type: str,
    country: str,
    median_distributions_file: Path = None,
    use_distributions_file: bool = False,
):
    """This function plots a boxplot with the distributions of the median fuel
    prices of a selected fuel type over time for a selected country.

    For each year a box for the border region stations and a box for the
    surrounding region stations is plottet.
    For the website use: if the median distribution file exists, you can
    set use_distributions_file to True and give the Path as parameter.
    Returns nothing.
    i: Path median_price_path, Path border_stations_file, string
    fuel_type, string country, Path median_distribution_file, bool
    use_distributions_file
    o: None
    """
    ...
    plot_df["year"] = pd.to_datetime(
        plot_df["year"].astype(int).astype(str), format="%Y"
    )
    ...
    fig.update_layout(
        title=(
            f"Yearly distribution of median {fuel_type} prices: border "
            f"vs. surrounding-region stations ({country})"
        ),
        xaxis_title="Year",
...
def perform_matched_panel_regression_autobahn_stations(
    ...
    """This method performs a panel regression that estimates the coefficient
    of the factor 'autobahn' on the price of a selected fuel type.

    It filters out the fixed effects of time and geographic proximity.
    For a robustness check, it's also possible to select the price
    median as the statistic. If needed, the function also returns the
    residual errors.
    i: Path median_price_path, Path stations_file, Path autobahn_file,
    string fuel_type, string statistic, bool return_residuals
    o: pd.DataFrame summary_df, pd.DataFrame analysis_panel,
    (pd.DataFrame residuals_df)
    """
    ...
    # create stations and autobahn lf and merge them into a df,
    # also classify the brand.
    stations_lf = pl.scan_csv(stations_file).select(
        pl.col(["uuid", "brand", "latitude", "longitude"])
    )
    autobahn_lf = (
        pl.scan_csv(autobahn_file)
        .select(pl.col(["uuid"]))
        .with_columns(pl.lit(1).alias("autobahn"))
    )

    stations_panel_df = (
        stations_lf.join(autobahn_lf, on="uuid", how="left")
        .with_columns(
            [
                pl.col("autobahn").fill_null(0).cast(pl.Int8),
                pl.col("brand")
                .map_elements(__classify_brand, return_dtype=pl.Utf8)
    ...
    price_panel_df = (
        pl.scan_parquet(price_files)
        .select(
            [
                pl.col("station_uuid").alias("uuid"),
                pl.col("day")
                .cast(pl.Utf8)
    ...
    # build the panel for the analysis
    # (combine the price and the stations dfs with the
    # match map helper method).
    match_map = __build_match_map(stations_panel_df, K_MATCHES, MAX_DISTANCE)
    analysis_panel = (
        match_map.join(
            stations_panel_df.select(
                ["uuid", "brand", "brand_category", "latitude", "longitude"]
            ),
    ...
    for year in sorted(panel_regression_df["year"].dropna().unique()):
        year_df = panel_regression_df[
            panel_regression_df["year"] == year
        ].copy()

        # define exogene variables
        # (-> also get dummies to the brand categories).
        exog = pd.DataFrame({"autobahn": year_df["autobahn"].astype(float)})
        brand_dummies = pd.get_dummies(
            year_df["brand_category"],
            prefix="brand",
            drop_first=True,
            dtype=float,
        )
        ...
        # define fixed effect that the modell needs to consider
        # (regional and time fixed).
        absorb = pd.DataFrame(
            {
                "match_set_uuid": pd.Categorical(year_df["match_set_uuid"]),
                "date": pd.Categorical(year_df["date"]),
            }
        )

        # define clustered standard error
        # (treat the errors from one station over time as one cluster
        # -> these errors are not independent from each other).
        clusters = pd.Categorical(year_df["station_uuid"]).codes.reshape(-1, 1)

        # define the model (we use abosrbingls).
        model = AbsorbingLS(
            dependent=year_df["y"].astype(float),
            exog=exog,
            absorb=absorb,
            drop_absorbed=True,
        )
        result = model.fit(
            cov_type="clustered", clusters=clusters, debiased=True
        )
...
def plot_yearly_autobahn_premium_line(
    yearly_df: pd.DataFrame, statistic: str = "mean"
):
    """This method plots a line of the autobahn premium for a selected fuel
    type with surrounding confidence intervall over the years.
    ...
    yearly_df["year"] = pd.to_datetime(
        yearly_df["year"].astype(int).astype(str), format="%Y"
    )
        ...
        buttons.append(
            dict(
                ...
                args=[
                    ...
                    {
                        "title": (
                            f"{fuel} price autobahnpremium with 95% "
                            "confidence interval over the years"
                        )
                    },
                ],
            )
    ...
    fig.update_layout(
        updatemenus=[
            dict(
                buttons=buttons, direction="down", showactive=True, x=1.05, y=1
            )
        ],
        xaxis_title="year",
        ...
        title=(
            f"{fuel_types[0]} price autobahnpremium with 95% confidence "
            "interval over the years"
        ),
    )
    fig.update_xaxes(tickformat="%Y", dtick="M12")
...
def plot_autobahn_premium_barchart(
    yearly_df: pd.DataFrame, statistic: str = "mean"
):
    """This function plots a bar chart where each bar represents one fuel type
    Autobahn premium in a year.

    So for each year, we have three separate bars.
    The price statistic is "mean" by default, but can also be changed to
    "median" if wanted. Returns nothing.
    i: pd.DataFrame yearly_df, string statistic
    o: None
    """
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title=dict(text="autobahn premium (€/liter)", font=dict(size=16))
        ),
        legend=dict(
            x=0,
            y=1.0,
...
# NOTE: parts of this function were written with the help of chatgpt.
def perform_wilcoxon_variance_test_on_autobahn(
    residuals_df: pd.DataFrame,
    measure: str = "mad",
    min_station_observations: int = 30,
):
    """This method perfoms a wilcoxon signed rank test on the residual errors
    of the fixed-effects panel regression for the autobahn premium.

    The test compares the paired difference of the residual station
    volatility for Autobahn and each average control group for each year.
    The standard measure the test uses is median absolute deviation, it
    can also use the standard deviation as measure.
    Also, the minimum required amount of observation per station,
    min_station_observations, can be changed.
    ...
    if measure not in ["mad", "sd"]:
        raise ValueError(
            "measure has to be median absolute deviation (mad) "
            "or standard deviation (sd)"
        )

    df = residuals_df.copy()

    # calculate the volatility per station
    # and extract those with suffiecient sample size.
    station_volatility = df.groupby(
        ["match_set_uuid", "station_uuid", "year", "autobahn"], as_index=False
    ).agg(
    ...
    station_volatility = station_volatility[
        station_volatility["n_obs"] >= min_station_observations
    ].copy()
    ...
    for year in sorted(station_volatility["year"].dropna().unique()):
        yearly_df = station_volatility[
            station_volatility["year"] == year
        ].copy()

        # create test and control group.
        test = yearly_df[yearly_df["autobahn"] == 1].rename(
            columns={volatility_col: "test_volatility"}
        )[["match_set_uuid", "test_volatility"]]

        controls = (
            yearly_df[yearly_df["autobahn"] == 0]
            .groupby("match_set_uuid", as_index=False)
            .agg(control_volatility=(volatility_col, "mean"))
        )

        # create test-control pairs and check that they're not empty
        # (-> if empty, skip for this year).
        paired = test.merge(
            controls, on="match_set_uuid", how="inner"
        ).dropna()
        if paired.empty:
            continue
        ...
        diff = np.round(
            (
                paired["test_volatility"] - paired["control_volatility"]
            ).to_numpy(),
            12,
        )

        # if there if no difference, drop the pair
        # (default zero handling for wilcoxon test).
        diff = diff[diff != 0]
...
def plot_wilcoxon_results_loolipop(wilcoxon_df: pd.DataFrame):
    """Plots a lollipop plot for the median volatility difference of autobahn
    station prices and non autobahn station prices.

    The difference is positive, when the autobahn volatility is higher
    than the non autobahn volatility. Returns nothing.
    ...
    wilcoxon_df["year"] = pd.to_datetime(
        wilcoxon_df["year"].astype(int).astype(str), format="%Y"
    )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            hovertemplate=(
                "Year: %{customdata[0]}<br>"
                "Median volatility difference: %{customdata[1]:.4f} "
                "€/liter<br>"
                "<extra></extra>"
            ),
        )
    ...
    fig.update_layout(
        title=(
            "Autobahn vs non autobahn stations median volatility "
...
# classify each brand into one of these categoties:
# brand, non brand, not defined/unknown.
# NOTE: this method was written with the help of chatgpt.
def __classify_brand(brand: str):
    """This method classifies the brand of a gas station using its name and a
    predefined list of buzzwords that indicate the brand class of a station.
...
# we match each autobahn station with a k-dim tree
# to its nearest non autobahn neighbours to get regional
# proximity for the controll groups.
# By doing this, we mitigate the risk of other geographical effects.
...
def __build_match_map(stations: pl.DataFrame, k: int, max_dist: int):
    ...
    # to avoid érrors, make sure k doesnt exceed number
    # of available controll stations.
    ...
    # for each autobahn station, query the tree
    # to get the closest non autobahn stations.
    ...
    match_map = pl.DataFrame(rows).unique(
        subset=["match_set_uuid", "station_uuid"]
    )
...
def save_png(fig, img_name: Path, legend: bool = False):
    ...
    Before saving the plot, the method adjusts the text to an
    appropriate size. If the plot has legend, set legend to True so it
    also adjusts the legends font size before saving.
    ...
    fig.update_layout(
        ...
        font=dict(size=94),
        title=dict(
            font=dict(size=100), y=0.99, x=0.5, xanchor="center", yanchor="top"
        ),
    )
    if legend:
        fig.update_layout(
    ...
    # change path to own pc
    img_path = Path(r"/Users/sebastian/data-science-projekt/rq_results")
    fig.write_image(img_path / img_name, width=px_w, height=px_h, scale=1)
...
if __name__ == "__main__":
    ...
    summany_file = Path(
        r"/Users/sebastian/data-science-projekt/rq_results/"
