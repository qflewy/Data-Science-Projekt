...
def display_weather_codes_per_region(
    weather_path: Path, weather_codes: Path, region: str, year: int
):
    """Displays a plotly scatterplot with the (hourly) weathercodes and their
    description from a chosen region in a chosen year.
    ...
    df = pd.read_csv(file_path).drop(
        columns=["precipitation", "temperature_2m"]
    )
...
def run_volatility_panel_regression(
    ...
    weather_path: Path,
    regions: list,
    extreme_weather_codes_file: Path,
    fuel_type: str = "diesel",
    statistic: str = "median",
    mad_window: int = 24,
):
    """Builds a panel dataset for each Leitregion and then estimates the effect
    of extreme weather on the gas stations price volatility.

    i: Path price_path, Path weather_path, list regions,
    Path extreme_weather_codes_file, string fuel_type, string statistic,
    int mad_window
    ...
    for region in regions:
        ...
        weather_lf = pl.scan_csv(weather_file, try_parse_dates=True).select(
            ["date", "weather_code"]
        )
        ...
        region_lf = (
            ...
            .with_columns(
                [
                    ...
                    # use a dummy variable for the weather codes
                    # (classify 1 as extreme, else 0).
                    pl.col("weather_code")
                    .is_in(extreme_codes)
                    .cast(pl.Int8)
                    .alias("extreme_weather"),
                    # time fixed effects (hourly and daily).
                    pl.col("timestamp_utc").dt.date().alias("date"),
                    pl.col("timestamp_utc").dt.hour().alias("hour"),
                    # get the price changes (first difference)
                    # from hour to hour.
            ...
            .with_columns(
                [
                    # calculate the rolling mean of price changes
                    # over the selected window.
                    pl.col("price_change")
                    .rolling_mean(
                        window_size=mad_window, min_samples=mad_window
                    )
                    .alias("rolling_mean_change")
                ]
            )
            .with_columns(
                [
                    # calculater the absolute deviation from the
                    # local rolling mean.
                    (pl.col("price_change") - pl.col("rolling_mean_change"))
                    .abs()
                    .alias("abs_deviation")
                ]
            )
    ...
    # because linearmodels requires a pandas df,
    # we convert our polars df into a pandas.
    panel_df = pl_panel_df.to_pandas()
    panel_df = panel_df.sort_values(["region", "timestamp_utc"]).reset_index(
        drop=True
    )
    ...
    # define the regional and time fixed effects
    # as factors the model has to absorb.
    ...
    # define the model
    # (we use the AbsorbingLS model with the above defined variables).
    model = AbsorbingLS(
        dependent=y, exog=x, absorb=absorbed_effects, drop_absorbed=True
    )
