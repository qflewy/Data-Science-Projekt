...
def build_station_daily_by_month(
    ...
    ----------
    data_root : Path | str
        Root folder containing subfolders "prices/{year}/{month}" with CSV
        price files.
    derived_root : Path | str
        Output root where "station_daily_last_by_month/{year}" subfolders are
        written.
    ...
    for YEAR in range(start_year, end_year + 1):
        ...
        OUT_YEAR_DIR = (
            DERIVED_ROOT
            / "station_daily_mean_and_median_by_month"
            / str(YEAR)
        )
        ...
        for month in range(1, 13):
            ...
            # Skip if file already exists (also helps to restart script if it
            ...
            # this is to fix the date parsing, because some dates have +01/+02
            # instead of +0100/+0200, which causes strptime to fail.
            ...
            # Sorting by month, station and day, then taking the
            # mean and median price of each day for each station
    ...
    print("All done. Everything went smoothly!")
