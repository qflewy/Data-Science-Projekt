...
def build_station_daily_by_month(
    data_root, derived_root, start_year, end_year
) -> None:
    ...
    ----------
    data_root : Path | str
        Root folder containing subfolders "prices/{year}/{month}" with
        CSV price files.
    derived_root : Path | str
        Output root where "station_daily_last_by_month/{year}"
        subfolders are written.
    ...
    for YEAR in range(start_year, end_year + 1):
        ...
        OUT_YEAR_DIR = (
            DERIVED_ROOT / "station_daily_mean_and_median_by_month" / str(YEAR)
        )
        ...
        for month in range(1, 13):
            ...
            # Skip if file already exists (also helps to restart script if
            ...
            lf = pl.scan_csv(
                in_glob, infer_schema_length=0, ignore_errors=True
            )
            ...
            # this is to fix the date parsing, because some dates have
            # +01/+02 instead of +0100/+0200, which causes strptime to fail.
            ...
            lf = (
                lf.with_columns(
                    ...
                    .str.strptime(
                        pl.Datetime, "%Y-%m-%d %H:%M:%S%z", strict=True
                    )
                    .alias("dt")
                )
                .with_columns(
                ...
                .with_columns([
                    pl.when(pl.col("diesel") <= 0)
                    .then(None)
                    .otherwise(pl.col("diesel"))
                    .alias("diesel"),
                    pl.when(pl.col("e5") <= 0)
                    .then(None)
                    .otherwise(pl.col("e5"))
                    .alias("e5"),
                    pl.when(pl.col("e10") <= 0)
                    .then(None)
                    .otherwise(pl.col("e10"))
                    .alias("e10"),
                ])
            )

            # Sorting by month, station and day, then taking the mean and
            # median
