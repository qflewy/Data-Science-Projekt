...
#Configurating the paths and years to process:
DATA_ROOT = Path(r"D:\data\tankerkoenig-data")
DERIVED_ROOT = Path(r"D:\data\derived")
for YEAR in range(2014, 2027):

    IN_YEAR_DIR = DATA_ROOT / "prices" / str(YEAR)
    OUT_YEAR_DIR = DERIVED_ROOT / "station_daily_last_by_month" / str(YEAR)
    ...
    for month in range(1, 13):
        ...
        # Sorting by month, station and day, then taking the last price of each day for each station
        # (which should be the most complete one, as stations usually update their prices throughout the day,
        # and the last one is usually the most complete one).
        station_daily_month = (
            lf.sort("dt")
            .group_by(["station_uuid", "day"])
            .agg([
                pl.col("diesel").drop_nulls().last().alias("diesel_last"),
                pl.col("e5").drop_nulls().last().alias("e5_last"),
                pl.col("e10").drop_nulls().last().alias("e10_last"),
